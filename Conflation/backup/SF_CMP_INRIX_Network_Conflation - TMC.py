...
inrix_net_prj["SegID"] = inrix_net_prj["TMC"]
inrix_net_prj["FRC"] = inrix_net_prj["Type"]
inrix_net_prj["RoadList"] = ""
        ...
        if intersect_b.empty:
            ...
            for link_idx in intersect_b_idx:
                ...
                if link_e_cmp_b_dis * 3.2808 / cmp_seg_len > 0.98:
                    ...
                    link_degree = (180 / math.pi) * math.atan2(
                        link_e_long - cmp_b_projected.x, link_e_lat - cmp_b_projected.y
                    )
                    cmp_degree = (180 / math.pi) * math.atan2(
                        link_e_projected.x - cmp_b_long, link_e_projected.y - cmp_b_lat
                    )
                    link_cmp_angle = abs(cmp_degree - link_degree)
                    if link_cmp_angle > 270:
                        link_cmp_angle = 360 - link_cmp_angle
...
inrix_lines_matched_output.to_csv(
    "CMP_Segment_INRIX_Links_Correspondence.csv", index=False
)
cmp_segs_prj.to_file(cmp + "_matchedlength_check.shp")
inrix_lines_matched_output.to_csv(
    "CMP_Segment_INRIX_Links_Correspondence.csv", index=False
