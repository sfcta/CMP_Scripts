...
with fiona.open(dtaout, mode='w', crs=cal3, driver='ESRI Shapefile', schema=vschema) as output:
    ...
    for line in layer:
        ...
        if isinstance(v_begin, list):
            ...
            prop = {'ID': int(line['properties']['ID']),
...
inrix_lines_intersect=gp.sjoin(dta_net_prj, cmp_segs_buffer, op='intersects').reset_index()

# INRIX lines within cmp segment buffer zone
inrix_lines_within=gp.sjoin(dta_net_prj, cmp_segs_buffer, op='within').reset_index()
...
inrix_lines_intersect_ep=gp.sjoin(dta_net_prj, cmp_endpoints_buffer, op='intersects').reset_index()
