...
group_cols = ['street','dow','timep','Month','Year','Date','Hour']
cmp_agg = df_cmp.groupby(group_cols).agg({'ttime': ['count', 'sum'], 'Miles': 'sum'}).reset_index().compute()
cmp_agg.columns = ['Street', 'DOW', 'Time_period', 'Month', 'Year','Date','Hour', 'Sample_size', 'Travel_time', 'Miles']
cmp_agg['Avg_speed'] = cmp_agg['Miles']/cmp_agg['Travel_time'] 
# cmp_agg['Miles'] = cmp_agg['Miles']/cmp_agg['Sample_size']

cmp_agg['Avg_speed'] = cmp_agg['Avg_speed']*cmp_agg['Sample_size']
cmp_agg = cmp_agg.groupby(['Street', 'DOW', 'Time_period', 'Month', 'Year'])['Sample_size','Avg_speed'].sum().reset_index()
cmp_agg['Avg_speed'] = cmp_agg['Avg_speed']/cmp_agg['Sample_size']
cmp_agg['Avg_speed'] = cmp_agg['Avg_speed'].round(1)
