...
# OUT_FILE = 'CMP 2019 Auto LOS v1.csv'

# SFCTA Paths
NETCONF_DIR = r'Q:\CMP\LOS Monitoring 2019\Network_Conflation\XD map 19_02'
DATA_DIR = r'Q:\Data\Observed\Streets\INRIX'
SPEED_DIR = r'Q:\CMP\LOS Monitoring 2019\Auto_LOS'
OUT_FILE = 'CMP 2019 Auto LOS XD1902.csv'
...
df_cmp = pd.DataFrame()
for i in range(1,8):
    df1 = dd.read_csv(os.path.join(DATA_DIR, 'SF_All_v1902_2019-04-01_to_2019-05-17_1_min_part_%s\data.csv' %i), assume_missing=True)
    if len(df1)>0:
...
cmp_segs_los.to_csv(os.path.join(SPEED_DIR, OUT_FILE), index=False)
