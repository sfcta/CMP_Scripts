...
def read_day_dir_counts_df(filepath, sheet_name, direction, day_num):
    ...
    return df.rename(
        columns={"    TIME": "TIME", "HOUR ": "HOUR_TOTALS"}
    ).set_index("TIME")
    ...
    while True:
        ...
        try:
            if weekday_only and not is_weekday(
                ...
                continue
            df = read_day_dir_counts_df(
                filepath, sheet_name, direction, day_num
            )
...
def read_sheet_mean_totals(
    ...
    vols_dir0 = map(
        np.mean,
        read_totals(
            filepath, workbook, sheet_name, 0, weekday_only=weekday_only
        ),
    )
    rows = [[sheet_name, dir0, *vols_dir0]]
    if dir1 != "O":
        vols_dir1 = map(
            np.mean, read_totals(filepath, workbook, sheet_name, 1)
        )
...
def rename_sheet_name_to_location(volumes_df, locations_filepath):
    locations = (
        pd.read_excel(locations_filepath)
        .set_index("Location ID")["Description"]
        .str.removesuffix("*")
    )
    volumes_df["2023_location_id"] = (
        volumes_df["sheet_name"].str.split("-", expand=True)[1].astype(int)
    )
...
def parse_wiltec_excel(
    raw_xlsx_filepath, locations_filepath, output_csv_filepath
):
    print("parsing workbook", raw_xlsx_filepath)
    rename_sheet_name_to_location(
        add_cmp_non_peak_volume(
            read_xlsx_volumes(raw_xlsx_filepath, weekday_only=True)
        ),
        locations_filepath,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "raw_xlsx_filepath", help="Wiltec-collected xlsx input filpath"
    )
    parser.add_argument(
        "locations_filepath",
        help=(
