...
# Get adjustment factors by FC
cmp_segs_los_inrix['ratio_17s_over_17f'] = cmp_segs_los_inrix['avg_speed_17s']/cmp_segs_los_inrix['avg_speed_17f']
cmp_segs_los_inrix['ratio_19f_over_17f'] = cmp_segs_los_inrix['avg_speed_19f'] / cmp_segs_los_inrix['avg_speed_17f']
fc_adjust_factors = cmp_segs_los_inrix.groupby(['period', 'cls_hcm00']).agg({'ratio_17s_over_17f': 'mean',
                                                                            'ratio_19f_over_17f': 'mean'}).reset_index()
fc_adjust_factors.columns=['period', 'cls_hcm00', 'fc_ratio_17s_over_17f', 'fc_ratio_19f_over_17f']

# Merge the aggregated FC adjustment factors to the speed table
cmp_segs_los_adj = pd.merge(cmp_segs_los, fc_adjust_factors, on = ['period', 'cls_hcm00'], how='left')


# Read in the floating car speeds
...
cmp_segs_los_adj['avg_speed_19s_adj'] = np.where(((cmp_segs_los_adj['source_17s']=='INRIX') & (cmp_segs_los_adj['source_17f']=='INRIX') & (cmp_segs_los_adj['source_19f']=='INRIX')),
                                                   ...
                                                   np.where(pd.isnull(cmp_segs_los_adj['avg_speed_19fcr']),
                                                           cmp_segs_los_adj['avg_speed_19f'] * cmp_segs_los_adj['fc_ratio_17s_over_17f'],
...
cmp_segs_los_adj['comment_19s_adj'] = np.where(((cmp_segs_los_adj['source_17s']=='INRIX') & (cmp_segs_los_adj['source_17f']=='INRIX') & (cmp_segs_los_adj['source_19f']=='INRIX')),
                                                   ...
                                                   np.where(pd.isnull(cmp_segs_los_adj['avg_speed_19fcr']),
                                                           'Adjusted based on 2019 Fall',
                                                           ' '))


# Fill the gaps in 2017 Spring speeds
cmp_segs_los_adj['avg_speed_17s_adj'] = np.where(cmp_segs_los_adj['source_17s']=='INRIX',
                                                 cmp_segs_los_adj['avg_speed_17s'],
                                                (cmp_segs_los_adj['avg_speed_19f']/cmp_segs_los_adj['fc_ratio_19f_over_17f']) * cmp_segs_los_adj['fc_ratio_17s_over_17f'])
...
# Determine 2019 Spring XD based LOS
...
# Determine 2017 Spring XD based LOS
...
# Save the large speed table for subsequent quality check
cmp_segs_los_adj.to_csv(os.path.join(MAIN_DIR, 'CMP XD1902 2017 vs 2019 Adjusted Comparison.csv'))
...
cmp_segs_los_adj_19s.to_csv(os.path.join(MAIN_DIR, 'CMP 2019 Auto XD LOS Adjusted v1.csv'), index=False)
...
cmp_segs_los_adj_17s.columns = ['cmp_id', 'year', 'source', 'period', 'avg_speed', 
                                         'los_hcm85', 'los_hcm00', 'sample_size', 'comment']
cmp_segs_los_adj_17s.to_csv(os.path.join(MAIN_DIR, 'CMP 2017 Auto XD LOS Adjusted v1.csv'), index=False)